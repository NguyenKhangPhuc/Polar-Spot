...
/**
 * PURPOSE:
 * Full-width hero banner component for single event detail view. Displays poster image preserving original aspect ratio
 * (object-contain) with ambient blur backdrop, dark gradient overlay, and overlaid event title & badges with Framer Motion entry animations.
 *
 * CONTEXT/PARENT FILE:
 * Extracted from app/events/[id]/SingleEventClient.tsx to isolate hero header banner UI.
 *
 * INPUTS / PARAMETERS:
 * - event (Event, Required): Single event record payload.
 * - posterUrl (string | null, Required): Fully qualified storage URL for poster image.
 */
...
export function HeroBanner({ event, posterUrl }: HeroBannerProps) {
   ...
   * Renders the standalone hero banner with object-contain main image and ambient blur background.
   * Uses motion.div for dark gradient title text overlay with slide-up and scale entry animation.
   *
   * PARAMETERS:
   * - props (HeroBannerProps): Component props object.
   *
   * RETURNS:
   * - JSX.Element: Hero banner element.
   */
  return (
    <motion.div
      ...
      className="relative w-full h-72 sm:h-96 md:h-[420px] rounded-2xl overflow-hidden border border-white/20 shadow-2xl bg-[#0a1526] flex items-center justify-center group"
    >
      {posterUrl ? (
        <>
          ...
          />
          {/* Main Poster Image Maintaining Exact Original Aspect Ratio */}
          <Image
            src={posterUrl}
            alt={eventTitle}
            fill
