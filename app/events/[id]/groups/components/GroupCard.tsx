...
export function GroupCard({ group, index = 0, canGrade = false, profile }: GroupCardProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="pt-4 border-t border-white/12 space-y-3 font-mono">
          {/* Row 1: 50% - 50% width grid (Grade the Group + Give Feedback) - Only visible for Admin & Judge */}
          {canGrade && (
            <div className="grid grid-cols-2 gap-3">
              <Link
                href={`/groups/${group.id}/grading`}
                className="w-full flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-md bg-[#3be1fe] hover:bg-[#6ee7fc] text-black font-bold text-xs font-mono uppercase tracking-wider transition-colors shadow-md cursor-pointer text-center"
              >
                ...
                <span className="truncate">GRADE GROUP</span>
              </Link>

              <button
                type="button"
                onClick={() => setIsFeedbackModalOpen(true)}
                className="w-full flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-md bg-[#050505] hover:bg-white/10 text-slate-200 border border-white/20 font-bold text-xs font-mono uppercase tracking-wider transition-colors shadow-md cursor-pointer text-center"
              >
          ...
          {/* Row 2: 100% width button (View Group Feedbacks) */}
          <Link
            href={`/groups/${group.id}/feedbacks`}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-md bg-[#000000] hover:bg-[#18181b] text-[#3be1fe] border border-[#3be1fe]/40 font-bold text-xs font-mono uppercase tracking-wider transition-colors shadow-md cursor-pointer text-center"
          >
            <svg className="w-4 h-4 text-[#3be1fe] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg>
            <span>VIEW GROUP FEEDBACKS</span>
          </Link>
        </div>
      </motion.div>
