...
/**
 * PURPOSE:
 * Modular route proxy handler protecting Group Grading pages (/groups/:id/grading).
 * Only permits access to authenticated users with role 'admin' or 'judge'/'judges'.
 *
 * CONTEXT/PARENT FILE:
 * Invoked by proxy.ts at project root when request pathname matches /groups/:id/grading pattern.
 *
 * INPUTS / PARAMETERS:
...
export async function groupGradingRoute({
    ...
    const isGradingRoute = /^\/groups\/[^/]+\/grading(\/.*)?$/.test(pathname);
